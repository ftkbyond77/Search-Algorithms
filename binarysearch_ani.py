...
def on_draw():
    window.clear()
    topic_label = pyglet.text.Label("Binary Serach", x=window.width // 2, y=window.height - 25,
                                     anchor_x='center', anchor_y='center', font_size=18, color=(255, 255, 255, 255))
    topic_label.draw()
    for i, number in enumerate(numbers):
        ...
        if left <= i <= right and not search_complete:
            color = (150, 0, 255)  # Violet for the current search interval
        elif i == mid and not search_complete:
            color = (240, 230, 140)  # khaki for the middle element
        ...
        label = pyglet.text.Label(str(number), x=x+width//2, y=y+height//2, anchor_x='center', anchor_y='center',color=(255, 0, 0, 255) ,batch=batch)
        label.draw()
